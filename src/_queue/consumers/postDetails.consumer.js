...
const PostDetailsConsumer = new Connnection(KEYS.AMQP_URI, KEYS.UPDATE_USER_POST_DETAILS,
  async (msg) => {
    ...
    if (msg !== null) {
      ...
      try {
    //    update records here
    const updatedCommunityRecords = await new CommunityService().updateMany({poster_id:id}, bodyData);
    const updatedPostsRecords = await new PostDetailsService().updateMany({poster_id:id}, bodyData);
    const updatedTweetRecords = await new TweetService().updateMany({poster_id:id}, bodyData);
    const updatedRepostRecords = await new RepostService().updateMany({poster_id:id}, bodyData);
