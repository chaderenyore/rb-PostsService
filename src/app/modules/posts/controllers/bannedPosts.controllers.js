...
const BannedPostsService = require("../services/banedPosts.services");
...
exports.bannPosts = async (req, res, next) => {
  try {
    ...
    for(let i; i < ids.length; i++){
        ...
        const deletedPosts = await new CommunityPostsService().delete({original_post_id: ids[i]});
        //delete from users posts
        const deletedUserPosts = await new PostsService().deleteOne({_id: ids[i]});
